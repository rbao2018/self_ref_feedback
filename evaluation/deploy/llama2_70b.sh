...
model_name_or_path="/mnt/data/huggingface/models/Llama-2-70b-chat-hf"
model_class="llama2"
while true;
do
  gpu_allocated=false
  for gpu in 0 2 4 6; do
      ...
      if [[ $last_allocated_diff -lt 180 ]]; then
        ...
        lmdeploy serve api_server $model_name_or_path --backend turbomind --model-format hf --server-name 0.0.0.0 --server-port 1008$gpu --model-name $model_class --cache-max-entry-count 0.9 --tp 2 --session-len 8192 > /tmp/deploy_$gpu.log 2>&1 &
        gpu_allocated=true
        # 更新GPU的最后一次分配任务的时间戳

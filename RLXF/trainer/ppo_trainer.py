...
from openrlhf.trainer.ppo_utils import (
        ...
        for episode in range(args.num_episodes):
            ...
            for rand_prompts in self.prompts_dataloader:
                ...
                if global_step % update_timesteps == 0:
                    output = self.tokenizer.decode(experience.sequences[0], skip_special_tokens=(global_step>3))
                ...
                if global_step % update_timesteps == 0:
                    ...
                    # logs/checkpoints
                    self.save_logs_and_checkpoints(args, global_step, pbar, status)
                pbar.update()
                global_step = global_step + 1

            self.strategy.save_model(
                self.ema_model if args.enable_ema else self.actor.model,
                self.tokenizer,
                osp.join(args.save_path, f"actor_epoch{episode}"),
            )
